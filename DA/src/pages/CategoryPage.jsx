...
// Import JSON data
import mobilephonesData from '../data/mobilephones.json';
...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    if (categoryName === 'mobilephones') {
      setItems(mobilephonesData);
      setLoading(false);
    } else if (categoryName === 'laptops') {
      setItems(laptopsData);
      setLoading(false);
  ...
  const categoryDisplay = categoryName === 'mobilephones' ? 'Mobile Phones' : 
                          categoryName === 'laptops' ? 'Laptops' :
